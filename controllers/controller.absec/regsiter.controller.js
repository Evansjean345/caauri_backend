const Register = require("../../models/model.absec/register.model");
...
//getOne blog
exports.getOneRegister = async (req, res) => {
  if (!ObjectID.isValid(req.params.id)) {
    return res.status(400).send(`ID unknown : ${req.params.id}`);
  } else {
    try {
      await Register.findById({ _id: req.params.id })
        .then((el) => res.status(200).json(el))
...
//modifyHome logic
exports.modifyRegister = async (req, res) => {
  if (!ObjectID.isValid(req.params.id)) {
    return res.status(400).send(`ID unknown : ${req.params.id}`);
  }
